            ...
            <ul class="nav nav-tabs" id="myTab2" role="tablist">
                ...
                <li class="nav-item">
                    <a class="nav-link {{ $loop->index === 0 ? 'active' : '' }}" id="home-tab2" data-toggle="tab"
                        href="#home-{{ $language->lang }}" role="tab" aria-controls="home" aria-selected="true">{{
                        $language->name }}</a>
                </li>
                @endforeach
            </ul>
                    ...
                    <div class="card-body">
                        <form action="{{ route('admin.home-section-setting.update') }}" method="POST">
                            ...
                            <div class="form-group">
                                <label for="" class="col-lg-12 p-0">{{ __('Category Section one') }}</label>
                            ...
                            <div class="form-group">
                                <label for="" class="col-lg-12 p-0">{{ __('Category Section Two') }}</label>
                            ...
                            <div class="form-group">
                                <label for="" class="col-lg-12 p-0">{{ __('Category Section Three') }}</label>
                                <select name="category_section_three" id="" class="form-control select2"
                                    style="width: 100%">
                                    <option value="">---{{ __('Select') }}---</option>
                                    @foreach ($categories as $category)
                                    <option {{ @$homeSectionSetting->category_section_three == $category->id ?
                                        'selected' : '' }}
                                        value="{{ $category->id }}">{{ $category->name }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group">
                                <label for="" class="col-lg-12 p-0">{{ __('Category Section Four') }}</label>
